...
use crate::{
    ...
    is_empty_object_literal, is_expression, is_identifier, node_is_synthesized, visit_node,
    BaseNodeFactory, Debug_, NodeFactory, ReadonlyTextRangeConcrete,
};
...
trait FlattenContext {
    ...
    fn visitor(&self, _node: &Node) -> Option<VisitResult /*<Node>*/> {
        None
    }
}
...
pub fn flatten_destructuring_assignment<'visitor, 'create_assignment_callback>(
    ...
    let visitor: Option<Rc<RefCell<dyn FnMut(&Node) -> VisitResult + 'visitor>>> =
        visitor.map(|visitor| {
            Rc::new(RefCell::new(visitor))
                as Rc<RefCell<dyn FnMut(&Node) -> VisitResult + 'visitor>>
        });
    let create_assignment_callback: Option<
        Rc<
            RefCell<
                dyn FnMut(&Node, &Node, Option<&dyn ReadonlyTextRange>) -> Gc<Node>
                    + 'create_assignment_callback,
            >,
        >,
...
struct FlattenDestructuringAssignmentFlattenContext<'visitor, 'create_assignment_callback> {
    ...
    create_assignment_callback: Option<
        ...
        >,
    >,
    visitor: Option<Rc<RefCell<dyn FnMut(&Node) -> VisitResult + 'visitor>>>,
}
...
impl FlattenContext for FlattenDestructuringAssignmentFlattenContext<'_, '_> {
    ...
    fn visitor(&self, node: &Node) -> Option<VisitResult /*<Node>*/> {
        self.visitor
            .as_ref()
