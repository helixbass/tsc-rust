...
impl TransformTypeScript {
    ...
    pub(super) fn visit_class_declaration(
        ...
        let name = released!(node.ref_(self).as_class_declaration().maybe_name()).or_else(|| {
                    ...
                    .create_variable_statement(
                        ...
                        self.factory.ref_(self).create_variable_declaration_list(
                            vec![self.factory.ref_(self).create_variable_declaration(
                                Some(self.factory.ref_(self).get_local_name(
                                    node,
                                    Some(false),
                                    Some(false),
                                )),
            ...
            || facts.intersects(ClassFacts::HasConstructorDecorators)
        {
            if facts.intersects(ClassFacts::IsDefaultExternalExport) {
                statements.push(
                    self.factory.ref_(self).create_export_default(
                        self.factory
                            .ref_(self)
    ...
    pub(super) fn get_decorated_class_elements(
        &self,
        node: Id<Node>, /*ClassExpression | ClassDeclaration*/
        is_static: bool,
    ) -> Vec<Id<Node /*ClassElement*/>> {
        node.ref_(self)
            .as_class_like_declaration()
            .members()
            .ref_(self)
    ...
    pub(super) fn get_decorators_of_parameters(
        ...
        if let Some(node) = node {
            ...
            let first_parameter_is_this = !parameters.ref_(self).is_empty()
                && parameter_is_this_keyword(parameters.ref_(self)[0], self);
            let first_parameter_offset = if first_parameter_is_this { 1 } else { 0 };
            let num_parameters = if first_parameter_is_this {
                parameters.ref_(self).len() - 1
            } else {
                parameters.ref_(self).len()
            };
            for i in 0..num_parameters {
                let parameter = parameters.ref_(self)[i + first_parameter_offset];
    ...
    pub(super) fn get_all_decorators_of_constructor(
        ...
        let parameters =
            self.get_decorators_of_parameters(get_first_constructor_with_body(node, self));
