...
use crate::{
    ...
    get_source_map_range, get_use_define_for_class_fields, insert_statement_after_custom_prologue,
    is_computed_property_name, is_identifier, is_private_identifier, is_property_name,
    set_emit_flags, set_source_map_range, try_visit_node, unescape_leading_underscores,
    AllAccessorDeclarations, Debug_, EmitFlags, GeneratedIdentifierFlags,
    NamedDeclarationInterface, Node, NodeArray, NodeExt, NodeInterface, NodeWrappered,
    PropertyDescriptorAttributesBuilder, ReadonlyTextRange, ReadonlyTextRangeConcrete, SyntaxKind,
    VisitResult,
};
...
impl TransformES2015 {
    ...
    pub(super) fn transform_accessors_to_expression(
        ...
        accessors: &AllAccessorDeclarations,
        _container: &Node,
        _starts_on_new_line: bool,
    ) -> Gc<Node /*Expression*/> {
        let _first_accessor = &accessors.first_accessor;
        let _get_accessor = accessors.get_accessor.as_ref();
        let _set_accessor = accessors.set_accessor.as_ref();
        unimplemented!()
    }
