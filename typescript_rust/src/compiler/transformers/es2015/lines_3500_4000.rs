...
impl TransformES2015 {
    ...
    pub(super) fn visit_catch_clause(
        ...
        if is_binding_pattern(
            ...
            let destructure = self
                .factory
                .ref_(self)
                .create_variable_statement(Option::<Id<NodeArray>>::None, list);
    ...
    pub(super) fn visit_type_script_class_wrapper(
        ...
        let class_statements = filter(&body_statements.ref_(self), |&statement: &Id<Node>| {
            self.is_variable_statement_with_initializer(statement)
        });
        let remaining_statements = filter(&body_statements.ref_(self), |&stmt: &Id<Node>| {
                ...
                == SyntaxKind::CommaToken
        {
            alias_assignment = try_cast(
                ...
                |&initializer_left: &Id<Node>| {
                    is_assignment_expression(initializer_left, None, self)
                },
            );
        }
        ...
        if let Some(alias_assignment) = alias_assignment {
            let extends_call = try_cast(
                func_statements.ref_(self).get(0).cloned(),
                |node: &Option<Id<Node>>| {
            ...
            statements.push(func_statements.ref_(self)[class_body_start].clone());
            class_body_start += 1;
        ...
        while !is_return_statement(
            &element_at(&func_statements.ref_(self), class_body_end)
                .unwrap()
                .ref_(self),
        ) {
            class_body_end -= 1;
        ...
        add_range(
            &mut statements,
            Some(&func_statements.ref_(self)),
    ...
    pub(super) fn transform_and_spread_elements(
        ...
        if segments.len() == 1 {
            ...
            if is_argument_list
                ...
                || is_packed_array_literal(first_segment.expression, self)
