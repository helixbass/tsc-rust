...
impl TypeChecker {
        ...
        if needs_extra_rest_element {
            ...
            if ptr::eq(shorter, right) {
                let type_ = self.maybe_instantiate_type(
                    (*self.symbol(rest_param_symbol).as_transient_symbol().symbol_links())
                        .borrow()
                        .type_,
                    mapper,
                )?;
    ...
    pub(super) fn resolve_union_type_members(
        ...
        let index_infos = self.get_union_index_infos(self.type_(type_).as_union_type().types())?;
        self.set_structured_type_members(
            self.type_(type_).as_union_type(),
            self.empty_symbols(),
            call_signatures,
    ...
    pub(super) fn resolve_intersection_type_members(
        ...
        self.set_structured_type_members(
            self.type_(type_).as_intersection_type(),
            self.empty_symbols(),
            call_signatures,
            construct_signatures,
            index_infos,
    ...
    pub(super) fn resolve_anonymous_type_members(
        ...
        if let Some(type_target) = {
        ...
        } else if self.symbol(symbol).flags().intersects(SymbolFlags::TypeLiteral) {
            self.set_structured_type_members(
                self.type_(type_).as_object_type(),
                self.empty_symbols(),
                vec![],
            ...
            if self.symbol(symbol).flags().intersects(SymbolFlags::Class) {
                ...
                if self.symbol(symbol).flags().intersects(SymbolFlags::Function) {
                    add_range(
    ...
    pub(super) fn resolve_reverse_mapped_type_members(
            ...
            self.get_properties_of_type(self.type_(type_).as_reverse_mapped_type().source)?
        {
            ...
            if self
                ...
                && self
                    .type_(
                        self.type_(type_constraint_type_type)
            ...
            let inferred_prop = self.alloc_symbol(inferred_prop
        ...
        self.set_structured_type_members(
            self.type_(type_).as_reverse_mapped_type(),
            Gc::new(GcCell::new(members)),
            vec![],
            vec![],
            index_infos,
