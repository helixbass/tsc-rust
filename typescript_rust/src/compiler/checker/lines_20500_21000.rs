...
impl TypeChecker {
    ...
    pub(super) fn get_single_base_for_non_augmenting_subtype(
        ...
        ) == 0
        {
            ...
            let target_type_parameters = target
                .ref_(self)
                .as_interface_type()
                .maybe_type_parameters()
    ...
    pub(super) fn create_symbol_with_type(
        ...
        *symbol.ref_(self).maybe_declarations_mut() =
        ...
        symbol_links.ref_mut(self).type_ = type_;
        symbol_links.ref_mut(self).target = Some(source);
        ...
        if let Some(name_type) = name_type {
            symbol_links.ref_mut(self).name_type = Some(name_type);
        }
        symbol
    }
