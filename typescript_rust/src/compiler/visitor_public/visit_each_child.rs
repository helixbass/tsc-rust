...
pub fn visit_each_child(
    ...
    maybe_visit_each_child(Some(node), visitor, context, arena).unwrap()
}
...
pub fn maybe_visit_each_child_full(
    ...
    let mut nodes_visitor = move |nodes: Option<Id<NodeArray>>,
        ...
        if let Some(nodes_visitor) = nodes_visitor.as_mut() {
            ...
            maybe_visit_nodes(nodes, visitor, test, start, count, arena)
        }
    };
    ...
    match kind {
        SyntaxKind::Identifier => Some(factory.ref_(arena).update_identifier(
            node,
            nodes_visitor(
                released!(node.ref_(arena).as_identifier().maybe_type_arguments()),
        ...
        SyntaxKind::TypeParameter => Some(
            factory.ref_(arena).update_type_parameter_declaration(
                ...
                node_visitor(
                    released!(node.ref_(arena).as_type_parameter_declaration().default),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                ),
        ...
        SyntaxKind::Parameter => {
            Some(factory.ref_(arena).update_parameter_declaration(
                node,
                nodes_visitor(
                ...
                node_visitor(
                    released!(node.ref_(arena).as_parameter_declaration().dot_dot_dot_token),
                    Some(&mut |node: Id<Node>| {
                        if let Some(token_visitor) = token_visitor.as_ref() {
                            token_visitor(node)
                        } else {
                            // TODO: this is bad this is just mimicking the behavior
                            // of visit_node() (in the case that this `visitor` arg isn't
                            // provided) (and hopefully any manually-provided `node_visitor`
                            // arguments also)
                            // Really ths argument to node_visitor should be None if
                            // `token_visitor` is None but running into a lot of lifetime pain
                            // (this applies to all uses of `token_visitor` in this function
                            // and similarly in try_maybe_visit_each_child_full())
                            Some(node.into())
                        }
                    }),
                ...
                node_visitor(
                    released!(node.ref_(arena).as_parameter_declaration().maybe_question_token()),
                    Some(&mut |node: Id<Node>| {
                        if let Some(token_visitor) = token_visitor.as_ref() {
                            token_visitor(node)
                        } else {
                ...
                node_visitor(
                    released!(node.ref_(arena).as_parameter_declaration().maybe_initializer()),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                ),
        ...
        SyntaxKind::Decorator => Some(
            factory.ref_(arena).update_decorator(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_decorator().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::PropertySignature => Some(
            factory.ref_(arena).update_property_signature(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_property_signature().name())),
                    Some(&mut visitor),
        ...
        SyntaxKind::PropertyDeclaration => Some(
            factory.ref_(arena).update_property_declaration(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_property_declaration().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_property_name(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::MethodSignature => Some(
            factory.ref_(arena).update_method_signature(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_method_signature().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_property_name(&node.ref_(arena))),
                    None,
                )
                ...
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_method_signature().parameters())),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::MethodDeclaration => Some(
            factory.ref_(arena).update_method_declaration(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_method_declaration().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_property_name(&node.ref_(arena))),
                    None,
                )
                ...
                visit_parameter_list_full(
                    released!(Some(node.ref_(arena).as_method_declaration().parameters())),
        ...
        SyntaxKind::Constructor => Some(
            factory.ref_(arena).update_constructor_declaration(
                ...
                visit_parameter_list_full(
                    released!(Some(
                        node.ref_(arena).as_constructor_declaration().parameters()
                    )),
                    |node: Id<Node>| visitor(node),
                    context,
                    Some(
        ...
        SyntaxKind::GetAccessor => Some(
            factory.ref_(arena).update_get_accessor_declaration(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_get_accessor_declaration().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_property_name(&node.ref_(arena))),
                    None,
                )
                ...
                visit_parameter_list_full(
                    released!(Some(
                        node.ref_(arena).as_get_accessor_declaration().parameters()
                    )),
                    |node: Id<Node>| visitor(node),
                    context,
                    Some(
        ...
        SyntaxKind::SetAccessor => Some(
            factory.ref_(arena).update_set_accessor_declaration(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_set_accessor_declaration().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_property_name(&node.ref_(arena))),
                    None,
                )
                ...
                visit_parameter_list_full(
                    released!(Some(
                        node.ref_(arena).as_set_accessor_declaration().parameters()
                    )),
                    |node: Id<Node>| visitor(node),
                    context,
                    Some(
        ...
        SyntaxKind::ClassStaticBlockDeclaration => {
            ...
            Some(
                factory.ref_(arena).update_class_static_block_declaration(
                    ...
                    visit_function_body_full(
                        released!(Some(node.ref_(arena).as_class_static_block_declaration().body)),
                        |node: Id<Node>| visitor(node),
                        context,
                        Some(
                            |node: Option<Id<Node>>,
        ...
        SyntaxKind::TypePredicate => Some(
            factory.ref_(arena).update_type_predicate_node(
                node,
                node_visitor(
                    released!(node.ref_(arena).as_type_predicate_node().asserts_modifier),
                    Some(&mut visitor),
                ...
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_type_predicate_node().maybe_type()),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                ),
        ...
        SyntaxKind::TypeReference => Some(
            factory.ref_(arena).update_type_reference_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_type_reference_node().type_name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_entity_name(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::FunctionType => Some(
            factory.ref_(arena).update_function_type_node(
                ...
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_function_type_node().parameters())),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_parameter_declaration(node, arena)),
                    None,
                    None,
        ...
        SyntaxKind::ConstructorType => Some(
            factory.ref_(arena).update_constructor_type_node(
                ...
                nodes_visitor(
                    released!(Some(
                        node.ref_(arena).as_constructor_type_node().parameters()
                    )),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_parameter_declaration(node, arena)),
                    None,
        ...
        SyntaxKind::TypeQuery => Some(
            factory.ref_(arena).update_type_query_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_type_query_node().expr_name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_entity_name(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::TypeLiteral => Some(
            factory.ref_(arena).update_type_literal_node(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_type_literal_node().members)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::ArrayType => Some(
            factory.ref_(arena).update_array_type_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_array_type_node().element_type)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::TupleType => Some(
            factory.ref_(arena).update_tuple_type_node(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_tuple_type_node().elements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                    None,
        ...
        SyntaxKind::OptionalType => Some(
            factory.ref_(arena).update_optional_type_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_optional_type_node().type_)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::RestType => Some(
            factory.ref_(arena).update_rest_type_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_rest_type_node().type_)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::UnionType => Some(
            factory.ref_(arena).update_union_type_node(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_union_type_node().types)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                    None,
        ...
        SyntaxKind::IntersectionType => Some(
            factory.ref_(arena).update_intersection_type_node(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_intersection_type_node().types)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                    None,
        ...
        SyntaxKind::ConditionalType => Some(
            factory.ref_(arena).update_conditional_type_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_conditional_type_node().check_type)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
                ...
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_conditional_type_node().true_type)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_conditional_type_node().false_type)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::InferType => Some(
            factory.ref_(arena).update_infer_type_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_infer_type_node().type_parameter)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_parameter_declaration(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::ImportType => Some(
            factory.ref_(arena).update_import_type_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_import_type_node().argument)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_import_type_node().qualifier),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_entity_name(&node.ref_(arena))),
                    None,
                ),
                ...
                released!(Some(node.ref_(arena).as_import_type_node().is_type_of())),
            ),
        ),
        SyntaxKind::NamedTupleMember => Some(
            factory.ref_(arena).update_named_tuple_member(
                node,
                node_visitor(
                    released!(node.ref_(arena).as_named_tuple_member().dot_dot_dot_token),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_dot_dot_dot_token(&node.ref_(arena))),
                    None,
                ),
                node_visitor(
                    released!(Some(node.ref_(arena).as_named_tuple_member().name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_named_tuple_member().question_token),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_question_token(&node.ref_(arena))),
                    None,
                ),
                node_visitor(
                    released!(Some(node.ref_(arena).as_named_tuple_member().type_)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::ParenthesizedType => Some(
            factory.ref_(arena).update_parenthesized_type(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_parenthesized_type_node().type_)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::TypeOperator => Some(
            factory.ref_(arena).update_type_operator_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_type_operator_node().type_)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::MappedType => Some(
            factory.ref_(arena).update_mapped_type_node(
                node,
                node_visitor(
                    released!(node.ref_(arena).as_mapped_type_node().readonly_token),
                    Some(&mut |node: Id<Node>| {
                        if let Some(token_visitor) = token_visitor.as_ref() {
                            token_visitor(node)
                        } else {
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_mapped_type_node().type_parameter)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_parameter_declaration(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_mapped_type_node().name_type),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                ),
                node_visitor(
                    released!(node.ref_(arena).as_mapped_type_node().question_token),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_question_or_plus_or_minus_token(&node.ref_(arena))),
                    None,
                ),
                node_visitor(
                    released!(node.ref_(arena).as_mapped_type_node().type_),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                ),
        ...
        SyntaxKind::LiteralType => Some(
            factory.ref_(arena).update_literal_type_node(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_literal_type_node().literal)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::TemplateLiteralType => Some(
            factory.ref_(arena).update_template_literal_type(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_template_literal_type_node().head)),
                    Some(&mut visitor),
        ...
        SyntaxKind::TemplateLiteralTypeSpan => Some(
            factory.ref_(arena).update_template_literal_type_span(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_template_literal_type_span().type_)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::ObjectBindingPattern => Some(
            factory.ref_(arena).update_object_binding_pattern(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_object_binding_pattern().elements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::ArrayBindingPattern => Some(
            factory.ref_(arena).update_array_binding_pattern(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_array_binding_pattern().elements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::BindingElement => Some(
            factory.ref_(arena).update_binding_element(
                node,
                node_visitor(
                    released!(node.ref_(arena).as_binding_element().dot_dot_dot_token),
                    Some(&mut |node: Id<Node>| {
                        if let Some(token_visitor) = token_visitor.as_ref() {
                            token_visitor(node)
                        } else {
                ...
                node_visitor(
                    released!(node.ref_(arena).as_binding_element().property_name),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_property_name(&node.ref_(arena))),
                    None,
                ),
                node_visitor(
                    released!(Some(node.ref_(arena).as_binding_element().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_binding_name(&node.ref_(arena))),
                    None,
                )
                ...
                .intersects(NodeFlags::OptionalChain)
            {
                return Some(
                    factory.ref_(arena).update_call_chain(
                        node,
                        node_visitor(
                            released!(Some(node.ref_(arena).as_call_expression().expression)),
                            Some(&mut visitor),
                            Some(&|node: Id<Node>| is_expression(node, arena)),
                            None,
                        )
                        .unwrap(),
                        node_visitor(
                            released!(node.ref_(arena).as_call_expression().question_dot_token),
                            Some(&mut |node: Id<Node>| {
                                if let Some(token_visitor) = token_visitor.as_ref() {
                                    token_visitor(node)
                                } else {
                        ...
                        nodes_visitor(
                            released!(Some(node.ref_(arena).as_call_expression().arguments)),
                            Some(&mut |node: Id<Node>| visitor(node)),
                            Some(&|node: Id<Node>| is_expression(node, arena)),
                            None,
                            None,
        ...
        SyntaxKind::NewExpression => Some(
            factory.ref_(arena).update_new_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_new_expression().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::TaggedTemplateExpression => Some(
            factory.ref_(arena).update_tagged_template_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_tagged_template_expression().tag)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::TypeAssertionExpression => Some(
            factory.ref_(arena).update_type_assertion(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_type_assertion().type_)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_type_assertion().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::FunctionExpression => Some(
            factory.ref_(arena).update_function_expression(
                ...
                visit_parameter_list_full(
                    released!(Some(node.ref_(arena).as_function_expression().parameters())),
                    |node: Id<Node>| visitor(node),
                    context,
                    Some(
                        |nodes: Option<Id<NodeArray>>,
        ...
        SyntaxKind::ArrowFunction => Some(
            factory.ref_(arena).update_arrow_function(
                ...
                visit_parameter_list_full(
                    released!(Some(node.ref_(arena).as_arrow_function().parameters())),
                    |node: Id<Node>| visitor(node),
                    context,
                    Some(
                        |nodes: Option<Id<NodeArray>>,
                ...
                visit_function_body_full(
                    released!(Some(
                        node.ref_(arena).as_arrow_function().maybe_body().unwrap()
                    )),
                    |node: Id<Node>| visitor(node),
                    context,
                    Some(
        ...
        SyntaxKind::DeleteExpression => Some(
            factory.ref_(arena).update_delete_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_delete_expression().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::TypeOfExpression => Some(
            factory.ref_(arena).update_type_of_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_type_of_expression().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::VoidExpression => Some(
            factory.ref_(arena).update_void_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_void_expression().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::AwaitExpression => Some(
            factory.ref_(arena).update_await_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_await_expression().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::PrefixUnaryExpression => Some(
            factory.ref_(arena).update_prefix_unary_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_prefix_unary_expression().operand)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::PostfixUnaryExpression => Some(
            factory.ref_(arena).update_postfix_unary_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_postfix_unary_expression().operand)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::BinaryExpression => Some(
            factory.ref_(arena).update_binary_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_binary_expression().left)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_binary_expression().operator_token)),
                    Some(&mut |node: Id<Node>| {
                        if let Some(token_visitor) = token_visitor.as_ref() {
                            token_visitor(node)
                        } else {
                ...
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_binary_expression().right)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::ConditionalExpression => Some(
            factory.ref_(arena).update_conditional_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_conditional_expression().condition)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                ...
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_conditional_expression().when_true)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::TemplateExpression => Some(
            factory.ref_(arena).update_template_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_template_expression().head)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_template_head(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::YieldExpression => Some(factory.ref_(arena).update_yield_expression(
            node,
            node_visitor(
                released!(node.ref_(arena).as_yield_expression().asterisk_token),
                Some(&mut |node: Id<Node>| {
                    if let Some(token_visitor) = token_visitor.as_ref() {
                        token_visitor(node)
                    } else {
            ...
            node_visitor(
                released!(node.ref_(arena).as_yield_expression().expression),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_expression(node, arena)),
                None,
            ),
        ...
        SyntaxKind::SpreadElement => Some(
            factory.ref_(arena).update_spread_element(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_spread_element().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::ClassExpression => Some(
            factory.ref_(arena).update_class_expression(
                ...
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_class_expression().members())),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::AsExpression => Some(
            factory.ref_(arena).update_as_expression(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_as_expression().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_as_expression().type_)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_type_node(&node.ref_(arena))),
                    None,
                )
                ...
                .intersects(NodeFlags::OptionalChain)
            {
                return Some(
                    factory.ref_(arena).update_non_null_chain(
                        node,
                        node_visitor(
                            released!(Some(node.ref_(arena).as_non_null_expression().expression)),
                            Some(&mut visitor),
                            Some(&|node: Id<Node>| is_expression(node, arena)),
                            None,
                        )
        ...
        SyntaxKind::MetaProperty => Some(
            factory.ref_(arena).update_meta_property(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_meta_property().name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::TemplateSpan => Some(
            factory.ref_(arena).update_template_span(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_template_span().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_template_span().literal)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_template_middle_or_template_tail(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::Block => Some(
            factory.ref_(arena).update_block(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_block().statements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::ExpressionStatement => Some(
            factory.ref_(arena).update_expression_statement(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_expression_statement().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::IfStatement => Some(
            factory.ref_(arena).update_if_statement(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_if_statement().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_if_statement().then_statement)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_statement(node, arena)),
                ...
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_if_statement().else_statement),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_statement(node, arena)),
                    Some(&|nodes: &[Id<Node>]| factory.ref_(arena).lift_to_block(nodes)),
                ),
        ...
        SyntaxKind::DoStatement => Some(
            factory.ref_(arena).update_do_statement(
                ...
                visit_iteration_body(
                    released!(node.ref_(arena).as_do_statement().statement),
                    |node: Id<Node>| visitor(node),
                    context,
                    arena,
                ),
                node_visitor(
                    released!(Some(node.ref_(arena).as_do_statement().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::WhileStatement => Some(
            factory.ref_(arena).update_while_statement(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_while_statement().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                visit_iteration_body(
                    released!(node.ref_(arena).as_while_statement().statement),
                    |node: Id<Node>| visitor(node),
                    context,
                    arena,
                ),
        ...
        SyntaxKind::ForStatement => Some(factory.ref_(arena).update_for_statement(
            node,
            node_visitor(
                released!(node.ref_(arena).as_for_statement().initializer),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_for_initializer(node, arena)),
                None,
            ),
            node_visitor(
                released!(node.ref_(arena).as_for_statement().condition),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_expression(node, arena)),
                None,
            ),
            node_visitor(
                released!(node.ref_(arena).as_for_statement().incrementor),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_expression(node, arena)),
                None,
            ),
            visit_iteration_body(
                released!(node.ref_(arena).as_for_statement().statement),
                |node: Id<Node>| visitor(node),
                context,
                arena,
            ),
        ...
        SyntaxKind::ForInStatement => Some(
            factory.ref_(arena).update_for_in_statement(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_for_in_statement().initializer)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_for_initializer(node, arena)),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_for_in_statement().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                visit_iteration_body(
                    released!(node.ref_(arena).as_for_in_statement().statement),
                    |node: Id<Node>| visitor(node),
                    context,
                    arena,
                ),
        ...
        SyntaxKind::ForOfStatement => Some(
            factory.ref_(arena).update_for_of_statement(
                node,
                node_visitor(
                    released!(node.ref_(arena).as_for_of_statement().await_modifier),
                    Some(&mut |node: Id<Node>| {
                        if let Some(token_visitor) = token_visitor.as_ref() {
                            token_visitor(node)
                        } else {
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_for_of_statement().initializer)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_for_initializer(node, arena)),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_for_of_statement().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                visit_iteration_body(
                    released!(node.ref_(arena).as_for_of_statement().statement),
                    |node: Id<Node>| visitor(node),
                    context,
                    arena,
                ),
        ...
        SyntaxKind::ContinueStatement => Some(factory.ref_(arena).update_continue_statement(
            node,
            node_visitor(
                released!(node.ref_(arena).as_continue_statement().label),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                None,
            ),
        ...
        SyntaxKind::BreakStatement => Some(factory.ref_(arena).update_break_statement(
            node,
            node_visitor(
                released!(node.ref_(arena).as_break_statement().label),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                None,
            ),
        ...
        SyntaxKind::ReturnStatement => Some(factory.ref_(arena).update_return_statement(
            node,
            node_visitor(
                released!(node.ref_(arena).as_return_statement().expression),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_expression(node, arena)),
                None,
            ),
        ...
        SyntaxKind::WithStatement => Some(
            factory.ref_(arena).update_with_statement(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_with_statement().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_with_statement().statement)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_statement(node, arena)),
                    Some(&|nodes: &[Id<Node>]| factory.ref_(arena).lift_to_block(nodes)),
                )
        ...
        SyntaxKind::SwitchStatement => Some(
            factory.ref_(arena).update_switch_statement(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_switch_statement().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_switch_statement().case_block)),
                    Some(&mut visitor),
        ...
        SyntaxKind::LabeledStatement => Some(
            factory.ref_(arena).update_labeled_statement(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_labeled_statement().label)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_labeled_statement().statement)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_statement(node, arena)),
                    Some(&|nodes: &[Id<Node>]| factory.ref_(arena).lift_to_block(nodes)),
                )
        ...
        SyntaxKind::ThrowStatement => Some(
            factory.ref_(arena).update_throw_statement(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_throw_statement().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::TryStatement => Some(
            factory.ref_(arena).update_try_statement(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_try_statement().try_block)),
                    Some(&mut visitor),
                ...
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_try_statement().catch_clause),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_catch_clause(&node.ref_(arena))),
                    None,
                ),
                node_visitor(
                    released!(node.ref_(arena).as_try_statement().finally_block),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_block(&node.ref_(arena))),
                    None,
                ),
        ...
        SyntaxKind::VariableDeclaration => Some(factory.ref_(arena).update_variable_declaration(
            ...
            node_visitor(
                released!(node.ref_(arena).as_variable_declaration().exclamation_token),
                Some(&mut |node: Id<Node>| {
                    if let Some(token_visitor) = token_visitor.as_ref() {
                        token_visitor(node)
                    } else {
            ...
            node_visitor(
                released!(node.ref_(arena).as_variable_declaration().maybe_initializer()),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_expression(node, arena)),
                None,
            ),
        ...
        SyntaxKind::FunctionDeclaration => Some(
            factory.ref_(arena).update_function_declaration(
                ...
                visit_parameter_list_full(
                    released!(Some(
                        node.ref_(arena).as_function_declaration().parameters()
                    )),
                    |node: Id<Node>| visitor(node),
                    context,
                    Some(
        ...
        SyntaxKind::ClassDeclaration => Some(
            factory.ref_(arena).update_class_declaration(
                ...
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_class_declaration().members())),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_class_element(&node.ref_(arena))),
                    None,
                    None,
        ...
        SyntaxKind::InterfaceDeclaration => Some(
            factory.ref_(arena).update_interface_declaration(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_interface_declaration().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
                ...
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_interface_declaration().members())),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_type_element(&node.ref_(arena))),
                    None,
                    None,
        ...
        SyntaxKind::TypeAliasDeclaration => Some(
            factory.ref_(arena).update_type_alias_declaration(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_type_alias_declaration().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::EnumDeclaration => Some(
            factory.ref_(arena).update_enum_declaration(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_enum_declaration().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_enum_declaration().members)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::ModuleDeclaration => Some(
            factory.ref_(arena).update_module_declaration(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_module_declaration().name())),
                    Some(&mut visitor),
                ...
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_module_declaration().body),
                    Some(&mut visitor),
        ...
        SyntaxKind::ModuleBlock => Some(
            factory.ref_(arena).update_module_block(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_module_block().statements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_statement(node, arena)),
                    None,
                    None,
        ...
        SyntaxKind::CaseBlock => Some(
            factory.ref_(arena).update_case_block(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_case_block().clauses)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::NamespaceExportDeclaration => Some(
            factory.ref_(arena).update_namespace_export_declaration(
                ...
                node_visitor(
                    released!(Some(
                        node.ref_(arena).as_namespace_export_declaration().name()
                    )),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
        ...
        SyntaxKind::ImportEqualsDeclaration => Some(
            factory.ref_(arena).update_import_equals_declaration(
                ...
                released!(node.ref_(arena).as_import_equals_declaration().is_type_only),
                node_visitor(
                    released!(Some(node.ref_(arena).as_import_equals_declaration().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::ImportDeclaration => Some(
            factory.ref_(arena).update_import_declaration(
                ...
                node_visitor(
                    released!(node.ref_(arena).as_import_declaration().import_clause),
                    Some(&mut visitor),
                ...
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_import_declaration().assert_clause),
                    Some(&mut visitor),
        ...
        SyntaxKind::AssertClause => Some(
            factory.ref_(arena).update_assert_clause(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_assert_clause().elements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                ...
                .unwrap(),
                released!(node.ref_(arena).as_assert_clause().multi_line),
            ),
        ),
        SyntaxKind::AssertEntry => Some(
            factory.ref_(arena).update_assert_entry(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_assert_entry().name)),
                    Some(&mut visitor),
                ...
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_assert_entry().value)),
                    Some(&mut visitor),
        ...
        SyntaxKind::ImportClause => Some(factory.ref_(arena).update_import_clause(
            node,
            released!(node.ref_(arena).as_import_clause().is_type_only),
            node_visitor(
                released!(node.ref_(arena).as_import_clause().name),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                None,
            ),
            node_visitor(
                released!(node.ref_(arena).as_import_clause().named_bindings),
                Some(&mut visitor),
        ...
        SyntaxKind::NamespaceImport => Some(
            factory.ref_(arena).update_namespace_import(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_namespace_import().name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::NamespaceExport => Some(
            factory.ref_(arena).update_namespace_export(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_namespace_export().name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::NamedImports => Some(
            factory.ref_(arena).update_named_imports(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_named_imports().elements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::ImportSpecifier => Some(
            factory.ref_(arena).update_import_specifier(
                node,
                released!(node.ref_(arena).as_import_specifier().is_type_only),
                node_visitor(
                    released!(node.ref_(arena).as_import_specifier().property_name),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                ),
                node_visitor(
                    released!(Some(node.ref_(arena).as_import_specifier().name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::ExportAssignment => Some(
            factory.ref_(arena).update_export_assignment(
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_export_assignment().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::ExportDeclaration => Some(factory.ref_(arena).update_export_declaration(
            ...
            released!(node.ref_(arena).as_export_declaration().is_type_only),
            node_visitor(
                released!(node.ref_(arena).as_export_declaration().export_clause),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_named_export_bindings(&node.ref_(arena))),
                None,
            ),
            node_visitor(
                released!(node.ref_(arena).as_export_declaration().module_specifier),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_expression(node, arena)),
                None,
            ),
            node_visitor(
                released!(node.ref_(arena).as_export_declaration().assert_clause),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_assert_clause(&node.ref_(arena))),
                None,
            ),
        ...
        SyntaxKind::NamedExports => Some(
            factory.ref_(arena).update_named_exports(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_named_exports().elements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::ExportSpecifier => Some(
            factory.ref_(arena).update_export_specifier(
                node,
                released!(node.ref_(arena).as_export_specifier().is_type_only),
                node_visitor(
                    released!(node.ref_(arena).as_export_specifier().property_name),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                ),
                node_visitor(
                    released!(Some(node.ref_(arena).as_export_specifier().name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::JsxElement => Some(
            factory.ref_(arena).update_jsx_element(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_jsx_element().opening_element)),
                    Some(&mut visitor),
                ...
                .unwrap(),
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_jsx_element().children)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                ...
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_jsx_element().closing_element)),
                    Some(&mut visitor),
        ...
        SyntaxKind::JsxOpeningElement => Some(
            factory.ref_(arena).update_jsx_opening_element(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_jsx_opening_element().tag_name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_jsx_tag_name_expression(&node.ref_(arena))),
                    None,
                )
                ...
                node_visitor(
                    released!(Some(node.ref_(arena).as_jsx_opening_element().attributes)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_jsx_attributes(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::JsxClosingElement => Some(
            factory.ref_(arena).update_jsx_closing_element(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_jsx_closing_element().tag_name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_jsx_tag_name_expression(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::JsxFragment => Some(
            factory.ref_(arena).update_jsx_fragment(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_jsx_fragment().opening_fragment)),
                    Some(&mut visitor),
                ...
                .unwrap(),
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_jsx_fragment().children)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_jsx_child(&node.ref_(arena))),
                    None,
                    None,
                ...
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_jsx_fragment().closing_fragment)),
                    Some(&mut visitor),
        ...
        SyntaxKind::JsxAttribute => Some(
            factory.ref_(arena).update_jsx_attribute(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_jsx_attribute().name)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_identifier(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_jsx_attribute().maybe_initializer()),
                    Some(&mut visitor),
        ...
        SyntaxKind::JsxAttributes => Some(
            factory.ref_(arena).update_jsx_attributes(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_jsx_attributes().properties)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::JsxSpreadAttribute => Some(
            factory.ref_(arena).update_jsx_spread_attribute(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_jsx_spread_attribute().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::JsxExpression => Some(factory.ref_(arena).update_jsx_expression(
            node,
            node_visitor(
                released!(node.ref_(arena).as_jsx_expression().expression),
                Some(&mut visitor),
                Some(&|node: Id<Node>| is_expression(node, arena)),
                None,
            ),
        ...
        SyntaxKind::CaseClause => Some(
            factory.ref_(arena).update_case_clause(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_case_clause().expression)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
                .unwrap(),
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_case_clause().statements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_statement(node, arena)),
                    None,
                    None,
        ...
        SyntaxKind::DefaultClause => Some(
            factory.ref_(arena).update_default_clause(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_default_clause().statements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_statement(node, arena)),
                    None,
                    None,
        ...
        SyntaxKind::HeritageClause => Some(
            factory.ref_(arena).update_heritage_clause(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_heritage_clause().types)),
                    Some(&mut |node: Id<Node>| visitor(node)),
        ...
        SyntaxKind::CatchClause => Some(
            factory.ref_(arena).update_catch_clause(
                node,
                node_visitor(
                    released!(node.ref_(arena).as_catch_clause().variable_declaration),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_variable_declaration(&node.ref_(arena))),
                    None,
                ),
                node_visitor(
                    released!(Some(node.ref_(arena).as_catch_clause().block)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_block(&node.ref_(arena))),
                    None,
                )
        ...
        SyntaxKind::PropertyAssignment => Some(
            factory.ref_(arena).update_property_assignment(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_property_assignment().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_property_name(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(Some(node.ref_(arena).as_property_assignment().initializer)),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                )
        ...
        SyntaxKind::EnumMember => Some(
            factory.ref_(arena).update_enum_member(
                node,
                node_visitor(
                    released!(Some(node.ref_(arena).as_enum_member().name())),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_property_name(&node.ref_(arena))),
                    None,
                )
                .unwrap(),
                node_visitor(
                    released!(node.ref_(arena).as_enum_member().maybe_initializer()),
                    Some(&mut visitor),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                ),
        ...
        SyntaxKind::CommaListExpression => Some(
            factory.ref_(arena).update_comma_list_expression(
                node,
                nodes_visitor(
                    released!(Some(node.ref_(arena).as_comma_list_expression().elements)),
                    Some(&mut |node: Id<Node>| visitor(node)),
                    Some(&|node: Id<Node>| is_expression(node, arena)),
                    None,
                    None,
