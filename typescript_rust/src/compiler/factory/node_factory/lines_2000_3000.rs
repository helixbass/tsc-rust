...
use crate::{
    ...
    HasTypeArgumentsInterface, HasTypeInterface, HasTypeParametersInterface, ImportTypeNode,
    InArena, IndexedAccessTypeNode, InferTypeNode, InterfaceOrClassLikeDeclarationInterface,
    LiteralTypeNode, MappedTypeNode, ModifierFlags, NamedDeclarationInterface, NewExpression, Node,
    NodeArray, NodeArrayOrVec, NodeFactory, NodeFlags, NodeInterface, ObjectBindingPattern,
    ObjectLiteralExpression, OptionInArena, ParenthesizedExpression, ParenthesizedTypeNode,
    PostfixUnaryExpression, PrefixUnaryExpression, PropertyAccessExpression,
    SignatureDeclarationInterface, SpreadElement, StrOrRcNode, StringOrNumberOrBoolOrRcNode,
...
impl NodeFactory {
    ...
    pub fn update_indexed_access_type_node(
        ...
        if node.ref_(self).as_indexed_access_type_node().object_type != object_type
            || node.ref_(self).as_indexed_access_type_node().index_type != index_type
        {
            self.update(
    ...
    pub fn update_object_binding_pattern(
        ...
        if has_node_array_changed(
            node.ref_(self).as_object_binding_pattern().elements,
            &elements,
        ) {
    ...
    pub fn update_type_of_expression(
        ...
        if node.ref_(self).as_type_of_expression().expression != expression {
    ...
    pub fn update_class_expression(
        ...
        if has_option_node_array_changed(node.ref_(self).maybe_decorators(), decorators.as_ref())
            || has_option_node_array_changed(node.ref_(self).maybe_modifiers(), modifiers.as_ref())
            || node.ref_(self).as_class_expression().maybe_name() != name
            || has_option_node_array_changed(
                node.ref_(self)
                    .as_class_expression()
                    .maybe_type_parameters(),
            ...
            || has_node_array_changed(node.ref_(self).as_class_expression().members(), &members)
        {
            self.update(
