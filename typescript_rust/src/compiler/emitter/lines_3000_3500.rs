...
impl Printer {
    ...
    pub(super) fn emit_throw_statement(
        ...
        self.emit_expression_with_leading_space(
            Some(&*node.as_throw_statement().expression),
            None,
        )?;
    ...
    pub(super) fn emit_try_statement(&self, node: &Node /*TryStatement*/) -> io::Result<()> {
        self.emit_token_with_comment(
    ...
    pub(super) fn emit_signature_and_body(
        ...
        mut emit_signature_head: impl FnMut(&Node) -> io::Result<()>,
    ) -> io::Result<()> {
        let node_as_function_like_declaration = node.as_function_like_declaration();
        let body = node_as_function_like_declaration.maybe_body();
        Ok(if let Some(body) = body.as_ref() {
            if is_block(body) {
    ...
    pub(super) fn emit_block_function_body(&self, body: &Node /*Block*/) -> io::Result<()> {
    ...
    pub(super) fn emit_module_block(&self, node: &Node /*ModuleBlock*/) -> io::Result<()> {
        self.push_name_generation_scope(Some(node));
        for_each(
    ...
    pub(super) fn emit_case_block(&self, node: &Node /*CaseBlock*/) -> io::Result<()> {
        self.emit_token_with_comment(
    ...
    pub(super) fn emit_import_clause(&self, node: &Node /*ImportClause*/) -> io::Result<()> {
    ...
    pub(super) fn emit_named_imports(&self, node: &Node /*NamedImports*/) -> io::Result<()> {
    ...
    pub(super) fn emit_assert_clause(&self, node: &Node /*AssertClause*/) -> io::Result<()> {
        self.emit_token_with_comment(
    ...
    pub(super) fn emit_assert_entry(&self, node: &Node /*AssertEntry*/) -> io::Result<()> {
    ...
    pub(super) fn emit_named_exports(&self, node: &Node /*NamedExports*/) -> io::Result<()> {
        self.emit_named_imports_or_exports(node)?;
    ...
    pub(super) fn emit_named_imports_or_exports(
        &self,
        node: &Node, /*NamedImportsOrExports*/
    ) -> io::Result<()> {
        self.write_punctuation("{");
        self.emit_list(
            Some(node),
