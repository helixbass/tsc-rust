...
impl Binder {
    pub(super) fn bind_worker(&self, node: Id<Node>) {
        match node.ref_(self).kind() {
            ...
            SyntaxKind::PropertyAccessExpression | SyntaxKind::ElementAccessExpression => {
                ...
                if is_in_js_file(Some(&expr.ref_(self)))
                    && self
                        .file()
                        .ref_(self)
                        .as_source_file()
                        ...
                        .is_some()
                    && is_module_exports_access_expression(expr, self)
                    ...
                    AssignmentDeclarationKind::Property => {
                            ...
                            && is_identifier(&expression.ref_(self))
                        {
                            let symbol = lookup_symbol_for_name(
                                ...
                                &expression.ref_(self).as_identifier().escaped_text,
                            );
            ...
            SyntaxKind::JSDocParameterTag => {
                if node.ref_(self).parent().ref_(self).kind() == SyntaxKind::JSDocSignature {
                    return self.bind_parameter(node);
                }
                if node.ref_(self).parent().ref_(self).kind() != SyntaxKind::JSDocTypeLiteral {
                    return;
                }
                let prop_tag = node;
    ...
    pub(super) fn bind_namespace_export_declaration(
        ...
        let diag = if !is_source_file(&node.ref_(self).parent().ref_(self)) {
            Some(&*Diagnostics::Global_module_exports_may_only_appear_at_top_level)
        } else if !is_external_module(&node.ref_(self).parent().ref_(self)) {
        ...
        if let Some(diag) = diag {
            self.file()
                ...
                .bind_diagnostics_mut()
                .push(
                    self.alloc_diagnostic(self.create_diagnostic_for_node(node, diag, None).into()),
                );
        } else {
    ...
    pub(super) fn bind_module_exports_assignment(
        ...
        if is_object_literal_expression(&assigned_expression.ref_(self)) {
            ...
            let assigned_expression_as_object_literal_expression =
                assigned_expression_ref.as_object_literal_expression();
            if every(
                &assigned_expression_as_object_literal_expression
                    .properties
                    .ref_(self),
                ...
                if is_binary_expression(&this_container.ref_(self).parent().ref_(self)) {
                        ...
                        == SyntaxKind::EqualsToken
                    {
                        ...
                        if is_bindable_static_access_expression(l, None, self)
                            && is_prototype_access(
                                l.ref_(self).as_has_expression().expression(),
                                self,
                            )
                        {
