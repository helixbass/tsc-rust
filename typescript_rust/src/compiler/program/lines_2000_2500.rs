...
use crate::{
    ...
    HasArena, HasStatementsInterface, InArena, LiteralLikeNodeInterface, ModifierFlags, Node,
    NodeArray, NodeFlags, NodeInterface, Program, ReadonlyTextRange, ResolvedProjectReference,
    ScriptKind, SortedArray, SourceFileLike, SyntaxKind,
};
...
impl Program {
    ...
    pub(super) fn get_bind_and_check_diagnostics_for_file_no_cache(
        ...
        let check_diagnostics = if include_bind_and_check_diagnostics {
            type_checker
                .ref_(self)
                .get_diagnostics(Some(source_file), cancellation_token)?
        } else {
            vec![]
        };
    ...
    pub(super) fn get_diagnostics_with_preceding_directives(
        ...
        let mut directives =
            create_comment_directives_map(&source_file.ref_(self), comment_directives);
    ...
    pub(super) fn try_get_and_cache_diagnostics(
        ...
        if let Some(source_file) = source_file {
            cache.per_file.get_or_insert_default_().insert(
                source_file.ref_(self).as_source_file().path().clone(),
                result.clone(),
            );
        } else {
    ...
    pub fn get_options_diagnostics_of_config_file(&self) -> Vec<Id<Diagnostic>> {
        ...
        let mut diagnostics = self.program_diagnostics().get_diagnostics(Some(
            &**options_config_file.ref_(self).as_source_file().file_name(),
        ));
        self.for_each_resolved_project_reference(
            |resolved_ref: Id<ResolvedProjectReference>| -> Option<()> {
                diagnostics.append(
                    &mut self.program_diagnostics().get_diagnostics(Some(
                        &**resolved_ref
                            .ref_(self)
                            .source_file
                            .ref_(self)
    ...
    pub fn module_name_is_equal_to(
        ...
        if a.ref_(self).kind() == SyntaxKind::Identifier {
            b.ref_(self).kind() == SyntaxKind::Identifier
                && a.ref_(self).as_identifier().escaped_text
                    == b.ref_(self).as_identifier().escaped_text
        } else {
            b.ref_(self).kind() == SyntaxKind::StringLiteral
                && &*a.ref_(self).as_literal_like_node().text()
                    == &*b.ref_(self).as_literal_like_node().text()
        }
    }
    ...
    pub fn create_synthetic_import(
        ...
        set_parent(
            &external_helpers_module_reference.ref_(self),
            Some(import_decl),
        );
        set_parent(&import_decl.ref_(self), Some(file));
        external_helpers_module_reference.ref_(self).set_flags(
            external_helpers_module_reference.ref_(self).flags() & !NodeFlags::Synthesized,
        );
        import_decl
            .ref_(self)
            .set_flags(import_decl.ref_(self).flags() & !NodeFlags::Synthesized);
    ...
    pub fn collect_external_module_references(&self, file: Id<Node> /*SourceFile*/) {
            ...
            && !file_as_source_file.is_declaration_file()
        {
            ...
            let jsx_import = get_jsx_runtime_import(
                get_jsx_implicit_import_base(&self.options.ref_(self), Some(&file.ref_(self)))
                    .as_deref(),
                &self.options.ref_(self),
            );
    ...
    pub(super) fn collect_dynamic_import_or_require_calls(
        ...
        for match_ in r.find_iter(&file.ref_(self).as_source_file().text()) {
            ...
            if is_java_script_file && is_require_call(node, true, self) {
                set_parent_recursive(Some(node), false, self);
                if let Some(node_arguments_0) = node
                    .ref_(self)
                    .as_call_expression()
                    .arguments
                    .ref_(self)
                ...
                node_arguments.ref_(self).len() >= 1
                    && is_string_literal_like(&node_arguments.ref_(self)[0].ref_(self))
            } {
                set_parent_recursive(Some(node), false, self);
                if let Some(node_arguments_0) = node
                    .ref_(self)
    ...
    pub(super) fn get_containing_child(
        ...
        if child.ref_(self).pos() <= position
            && (position < child.ref_(self).end()
                || position == child.ref_(self).end()
                    && child.ref_(self).kind() == SyntaxKind::EndOfFileToken)
        {
            return Some(child);
        }
        None
