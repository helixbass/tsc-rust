...
impl ParserType {
    pub(super) fn parse_jsdoc_function_type(&self) -> Gc<Node> {
        ...
        if self.look_ahead_bool(|| self.next_token_is_open_paren()) {
            ...
            return self.with_jsdoc(
                self.finish_node(
                    self.factory()
                        .create_jsdoc_function_type(self, parameters, type_),
                    pos,
                    None,
                )
                .wrap(),
    ...
    pub(super) fn parse_jsdoc_type(&self) -> Gc<Node> {
        ...
        if has_dot_dot_dot {
            type_ = self
                .finish_node(
                    self.factory().create_jsdoc_variadic_type(self, Some(type_)),
                    pos,
                    None,
                )
                .wrap();
        ...
        if self.token() == SyntaxKind::EqualsToken {
            ...
            return self
                .finish_node(
                    self.factory().create_jsdoc_optional_type(self, Some(type_)),
                    pos,
                    None,
                )
                .wrap();
    ...
    pub(super) fn parse_signature_member(
        ...
        let node: Node = if kind == SyntaxKind::CallSignature {
            self.factory()
                .create_call_signature(self, type_parameters, parameters, type_)
                .into()
        } else {
            self.factory()
                .create_construct_signature(self, type_parameters, parameters, type_)
                .into()
        };
    ...
    pub(super) fn parse_mapped_type_parameter(&self) -> TypeParameterDeclaration {
        ...
        self.finish_node(
            self.factory()
                .create_type_parameter_declaration(self, name.wrap(), Some(type_), None),
            pos,
            None,
        )
    }
    ...
    pub(super) fn parse_tuple_element_type(&self) -> Gc<Node /*TypeNode*/> {
        ...
        if self.parse_optional(SyntaxKind::DotDotDotToken) {
            return self
                .finish_node(
                    self.factory()
                        .create_rest_type_node(self, self.parse_type()),
                    pos,
                    None,
                )
                .wrap();
    ...
    pub(super) fn parse_parenthesized_type(&self) -> Node /*TypeNode*/ {
        ...
        self.parse_expected(SyntaxKind::CloseParenToken, None, None);
        self.finish_node(
            self.factory().create_parenthesized_type(self, type_).into(),
            pos,
            None,
        )
    }
    ...
    pub(super) fn parse_modifiers_for_constructor_type(
        ...
        if self.token() == SyntaxKind::AbstractKeyword {
            ...
            let modifier = self.finish_node(
                self.factory()
                    .create_token(self, SyntaxKind::AbstractKeyword),
                pos,
                None,
            );
    ...
    pub(super) fn parse_function_or_constructor_type(&self) -> Gc<Node /*TypeNode*/> {
        ...
        let node: Node = if is_constructor_type {
            self.factory()
                .create_constructor_type_node(self, modifiers, type_parameters, parameters, type_)
                .into()
        } else {
            let function_type_node: Node = self
                .factory()
                .create_function_type_node(self, type_parameters, parameters, type_)
                .into();
                ...
                SyntaxKind::OpenBracketToken => {
                    ...
                    if self.is_start_of_type(None) {
                        ...
                        type_ = self
                            .finish_node(
                                self.factory()
                                    .create_indexed_access_type_node(self, type_, index_type),
                                pos,
                                None,
                            )
                            .wrap();
                        ...
                        type_ = self
                            .finish_node(
                                self.factory().create_array_type_node(self, type_),
                                pos,
                                None,
                            )
                            .wrap();
