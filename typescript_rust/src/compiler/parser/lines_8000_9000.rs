...
impl<'parser> ParseJSDocCommentWorker<'parser> {
    ...
    pub(super) fn parse_jsdoc_link(&self, start: usize) -> Option<Node> {
        ...
        let mut name: Option<Id<Node /*EntityName | JSDocMemberName*/>> =
            if token_is_identifier_or_keyword(self.parser.token()) {
                Some(self.parser.parse_entity_name(true, None).alloc(self))
            } else {
                None
            };
        if name.is_some() {
            while self.parser.token() == SyntaxKind::PrivateIdentifier {
                ...
                name = Some(
                    self.parser
                        .finish_node(
                            self.parser.factory().create_jsdoc_member_name_raw(
                                name.clone().unwrap(),
                                self.parser.parse_identifier(None, None).alloc(self),
                            ),
                            p2,
                            None,
                        )
    ...
    pub(super) fn parse_unknown_tag(
        ...
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_unknown_tag_raw(
                tag_name,
    ...
    pub(super) fn parse_parameter_or_property_tag(
        ...
        if let Some(nested_type_literal) = nested_type_literal {
            type_expression = Some(nested_type_literal.alloc(self));
            is_name_first = true;
        }
        let result = if target == PropertyLikeParse::Property {
            self.parser.factory().create_jsdoc_property_tag_raw(
                Some(tag_name),
                name,
                is_bracketed,
                type_expression,
            ...
            self.parser.factory().create_jsdoc_parameter_tag_raw(
                Some(tag_name),
                name,
                is_bracketed,
                type_expression,
        ...
        if let Some(type_expression) = type_expression.filter(|type_expression| {
            ...
            while let Some(child) = {
                ...
                if matches!(
                    ...
                    append(children.as_mut().unwrap(), Some(child.alloc(self)));
                }
            }
            if let Some(children) = children {
                let literal = self.parser.finish_node(
                    self.parser.factory().create_jsdoc_type_literal_raw(
                        Some(children),
                        Some(
                ...
                return Some(
                    self.parser.finish_node(
                        self.parser
                            .factory()
                            .create_jsdoc_type_expression_raw(literal.alloc(self)),
                        pos,
                        None,
                    ),
                );
    ...
    pub(super) fn parse_return_tag(
        ...
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_return_tag_raw(
                Some(tag_name),
                type_expression,
                self.parse_trailing_tag_comments(
                    start,
    ...
    pub(super) fn parse_type_tag(
        ...
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_type_tag_raw(
                Some(tag_name),
    ...
    pub(super) fn parse_see_tag(
        ...
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_see_tag_raw(
                Some(tag_name),
                name_expression,
                comments,
            ),
    ...
    pub(super) fn parse_implements_tag(
        ...
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_implements_tag_raw(
                Some(tag_name),
                class_name.alloc(self),
                self.parse_trailing_tag_comments(
                    start,
                    self.parser.get_node_pos().try_into().unwrap(),
                    margin,
    ...
    pub(super) fn parse_augments_tag(
        ...
        let class_name = self.parse_expression_with_type_arguments_for_augments();
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_augments_tag_raw(
                Some(tag_name),
                class_name.alloc(self),
                self.parse_trailing_tag_comments(
                    start,
    ...
    ) -> ExpressionWithTypeArguments /* & { expression: Identifier | PropertyAccessEntityNameExpression }*/
    {
        ...
        let node = self
            .parser
            .factory()
            .create_expression_with_type_arguments_raw(expression.alloc(self), type_arguments);
    ...
    pub(super) fn parse_property_access_entity_name_expression(&self) -> Node /* Identifier | PropertyAccessEntityNameExpression */
    {
        ...
        while self.parser.parse_optional(SyntaxKind::DotToken) {
            let name: Id<Node> = self.parse_jsdoc_identifier_name(None).alloc(self);
            node = self
                .parser
                .finish_node(
                    self.parser
                        .factory()
                        .create_property_access_expression_raw(node.alloc(self), name),
                    pos,
                    None,
                )
                .into();
    ...
    pub(super) fn parse_this_tag(
        ...
        self.skip_whitespace();
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_this_tag_raw(
                Some(tag_name),
                Some(type_expression),
                self.parse_trailing_tag_comments(
                    start,
    ...
    pub(super) fn parse_enum_tag(
        ...
        self.skip_whitespace();
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_enum_tag_raw(
                Some(tag_name),
                Some(type_expression),
                self.parse_trailing_tag_comments(
                    start,
    ...
    pub(super) fn parse_typedef_tag(
        ...
        if match type_expression {
            ...
            while let Some(child) = self
                .parser
                .try_parse(|| self.parse_child_property_tag(indent).map(|node| node.alloc(self)))
            {
                has_children = true;
        ...
        let typedef_tag = self.parser.factory().create_jsdoc_typedef_tag_raw(
            Some(tag_name),
            type_expression,
            full_name,
            comment,
    ...
    pub(super) fn parse_jsdoc_type_name_with_namespace(
        ...
        if self.parser.parse_optional(SyntaxKind::DotToken) {
            ...
            let js_doc_namespace_node = self.parser.factory().create_module_declaration_raw(
                Option::<Gc<NodeArray>>::None,
                Option::<Gc<NodeArray>>::None,
                type_name_or_namespace_name.alloc(self),
                body.map(|node| node.alloc(self)),
                if nested {
    ...
    pub(super) fn parse_callback_tag(
        ...
        let return_tag = self.parser.try_parse(|| {
            if self.parse_optional_jsdoc(SyntaxKind::AtToken) {
                ...
                tag.kind() == SyntaxKind::JSDocReturnTag {
                    return Some(tag.alloc(self));
                }
            }
            None
        });
        let type_expression = self.parser.finish_node(
            self.parser.factory().create_jsdoc_signature_raw(
                Option::<Gc<NodeArray>>::None,
                parameters,
                return_tag,
            ),
        ...
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_callback_tag_raw(
                Some(tag_name),
                type_expression.alloc(self),
                full_name,
                comment,
            ),
            start.try_into().unwrap(),
    ...
    pub(super) fn try_parse_child_tag(
        ...
        match &*tag_name.as_identifier().escaped_text {
            "type" => {
                return if target == PropertyLikeParse::Property {
                    Some(
                        self.parse_type_tag(start, tag_name.alloc(self), None, None)
                            .into(),
                    )
                } else {
                    None
        ...
        Some(
            self.parse_parameter_or_property_tag(start, tag_name.alloc(self), target, indent)
                .into(),
        )
    }
    ...
    pub(super) fn parse_template_tag_type_parameter(&self) -> Option<TypeParameterDeclaration> {
        ...
        Some(
            self.parser.finish_node(
                self.parser.factory().create_type_parameter_declaration_raw(
                    name,
                    None,
                    default_type,
                ),
    ...
    pub(super) fn parse_template_tag(
        ...
        self.parser.finish_node(
            self.parser.factory().create_jsdoc_template_tag_raw(
                Some(tag_name),
                constraint,
                type_parameters,
                self.parse_trailing_tag_comments(
    ...
    pub(super) fn parse_jsdoc_entity_name(&self) -> Node /*EntityName*/ {
        ...
        while self.parser.parse_optional(SyntaxKind::DotToken) {
            ...
            entity = self
                .parser
                .create_qualified_name(entity.alloc(self), name.alloc(self))
                .into();
        }
        entity
    }
