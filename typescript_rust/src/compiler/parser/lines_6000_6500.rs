...
impl ParserType {
    pub(super) fn parse_default_clause(&self) -> DefaultClause {
        ...
        self.finish_node(self.factory().create_default_clause(statements), pos, None)
    }
    ...
    pub(super) fn parse_case_block(&self) -> CaseBlock {
        ...
        self.finish_node(self.factory().create_case_block(clauses), pos, None)
    }

    pub(super) fn parse_switch_statement(&self) -> Gc<Node /*SwitchStatement*/> {
        let pos = self.get_node_pos();
        ...
        self.with_jsdoc(
            self.finish_node(
                self.factory()
                    .create_switch_statement(expression, case_block.wrap()),
                pos,
                None,
            )
            .wrap(),
    ...
    pub(super) fn parse_throw_statement(&self) -> Gc<Node /*ThrowStatement*/> {
        let pos = self.get_node_pos();
        let has_jsdoc = self.has_preceding_jsdoc_comment();
        self.parse_expected(SyntaxKind::ThrowKeyword, None, None);

        let mut expression: Option<Gc<Node>> = if self.scanner().has_preceding_line_break() {
            None
        } else {
        ...
        self.with_jsdoc(
            self.finish_node(self.factory().create_throw_statement(expression), pos, None)
                .wrap(),
            has_jsdoc,
        )
    }

    pub(super) fn parse_try_statement(&self) -> Gc<Node /*TryStatement*/> {
        let pos = self.get_node_pos();
        let has_jsdoc = self.has_preceding_jsdoc_comment();
        ...
        let mut finally_block: Option<Gc<Node>> = None;
        ...
        self.with_jsdoc(
            self.finish_node(
                self.factory()
                    .create_try_statement(try_block, catch_clause, finally_block),
                pos,
                None,
            )
            .wrap(),
    ...
    pub(super) fn parse_catch_clause(&self) -> CatchClause {
        ...
        self.finish_node(
            self.factory()
                .create_catch_clause(variable_declaration, block),
            pos,
            None,
        )
    }

    pub(super) fn parse_debugger_statement(&self) -> Gc<Node /*DebuggerStatement*/> {
        let pos = self.get_node_pos();
        let has_jsdoc = self.has_preceding_jsdoc_comment();
        self.parse_expected(SyntaxKind::DebuggerKeyword, None, None);
        self.parse_semicolon();
        self.with_jsdoc(
            self.finish_node(self.factory().create_debugger_statement(), pos, None)
                .wrap(),
            has_jsdoc,
        )
    }
    ...
    pub(super) fn parse_expression_or_labeled_statement(
        &self,
    ) -> Gc<Node /*ExpressionStatement | LabeledStatement*/> {
        let pos = self.get_node_pos();
        ...
        if is_identifier(&expression) && self.parse_optional(SyntaxKind::ColonToken) {
            node = self
                .factory()
                .create_labeled_statement(expression, self.parse_statement())
                .into();
        } else {
            if !self.try_parse_semicolon() {
                self.parse_error_for_missing_semicolon_after(&expression);
    ...
    pub(super) fn parse_statement(&self) -> Gc<Node> {
        match self.token() {
    ...
    pub(super) fn parse_declaration(&self) -> Gc<Node /*Statement*/> {
        let is_ambient = some(
            self.look_ahead(|| {
    ...
    pub(super) fn try_reuse_ambient_declaration(&self) -> Option<Gc<Node /*Statement*/>> {
    ...
    pub(super) fn parse_array_binding_element(&self) -> Node /*ArrayBindingElement*/ {
        ...
        if self.token() == SyntaxKind::CommaToken {
            return self.finish_node(self.factory().create_omitted_expression().into(), pos, None);
        }
    ...
    pub(super) fn parse_object_binding_element(&self) -> BindingElement {
        ...
        let mut property_name: Option<Gc<Node>> = Some(self.parse_property_name().wrap());
        let name: Gc<Node>;
