...
pub fn for_each_child_returns<TReturn>(
    ...
    match &*node.ref_(arena) {
        ...
        .or_else(|| {
            visit_nodes_returns(&mut cb_node, cb_nodes.as_mut(), node.maybe_modifiers().as_deref(), arena).or_else(
                || {
                    visit_nodes_returns(
                        &mut cb_node,
                        cb_nodes.as_mut(),
        ...
        .or_else(|| {
            visit_nodes_returns(&mut cb_node, cb_nodes.as_mut(), node.maybe_modifiers().as_deref(), arena)
        ...
        Node::NewExpression(node) => visit_node_returns(&mut cb_node, Some(node.expression))
            .or_else(|| {
                ...
                .or_else(|| {
                    visit_nodes_returns(&mut cb_node, cb_nodes.as_mut(), node.arguments.as_deref(), arena)
                })
            }),
        ...
        Node::JSDoc(node) => {
            ...
            .or_else(|| visit_nodes_returns(&mut cb_node, cb_nodes.as_mut(), node.tags.as_deref(), arena))
        }
...
pub fn try_for_each_child_returns<TReturn, TError>(
    ...
    Ok(match &*node.ref_(arena) {
        ...
        .try_or_else(|| {
            try_visit_nodes_returns(&mut cb_node, cb_nodes.as_mut(), node.maybe_modifiers().as_deref(), arena)?.try_or_else(
                || {
        ...
        Node::PropertyDeclaration(node) => {
            ...
            .try_or_else(|| {
                try_visit_nodes_returns(&mut cb_node, cb_nodes.as_mut(), node.maybe_modifiers().as_deref(), arena)?
                    .try_or_else(|| {
                        try_visit_node_returns(&mut cb_node, Some(node.name()))?.try_or_else(|| {
                            try_visit_node_returns(&mut cb_node, node.question_token.clone())?.try_or_else(
                                || {
        ...
        Node::NewExpression(node) => try_visit_node_returns(&mut cb_node, Some(node.expression))?
            .try_or_else(|| {
                ...
                .try_or_else(|| {
                    try_visit_nodes_returns(&mut cb_node, cb_nodes.as_mut(), node.arguments.as_deref(), arena)
                })
            })?,
        ...
        Node::JSDoc(node) => {
            ...
            .try_or_else(|| try_visit_nodes_returns(&mut cb_node, cb_nodes.as_mut(), node.tags.as_deref(), arena))?
        }
        Node::JSDocSeeTag(node) => {
            try_visit_node_returns(&mut cb_node, Some(node.tag_name()))?.try_or_else(|| {
