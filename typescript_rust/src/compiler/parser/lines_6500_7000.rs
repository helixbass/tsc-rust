...
impl ParserType {
    pub(super) fn parse_object_binding_pattern(&self) -> ObjectBindingPattern {
        ...
        self.finish_node(
            self.factory().create_object_binding_pattern(elements),
            pos,
            None,
        )
    }
    ...
    pub(super) fn parse_array_binding_pattern(&self) -> ArrayBindingPattern {
        ...
        self.finish_node(
            self.factory().create_array_binding_pattern(elements),
            pos,
            None,
        )
    }
    ...
    pub(super) fn parse_variable_declaration(
        ...
        let mut exclamation_token: Option<Gc<Node>> = None;
    ...
    pub(super) fn parse_variable_declaration_list(
        ...
        self.finish_node(
            self.factory()
                .create_variable_declaration_list(declarations, Some(flags)),
            pos,
            None,
        )
    }
    ...
    pub(super) fn parse_variable_statement(
        ...
        let node = self
            .factory()
            .create_variable_statement(modifiers, declaration_list.wrap());
    ...
    pub(super) fn parse_function_declaration(
        ...
        let modifier_flags = modifiers_to_flags(modifiers.as_double_deref());
    ...
    pub(super) fn parse_property_declaration(
        ...
        let exclamation_token: Option<Gc<Node>> =
        ...
        let node: Gc<Node> = self.factory().create_property_declaration(
            decorators,
            modifiers,
            name,
    ...
    pub(super) fn parse_property_or_method_declaration(
        ...
        decorators: Option<Gc<NodeArray>>,
        modifiers: Option<Gc<NodeArray>>,
    ) -> Gc<Node /*PropertyDeclaration | MethodDeclaration*/> {
    ...
    pub(super) fn parse_accessor_declaration(
        ...
        let node: Node = if kind == SyntaxKind::GetAccessor {
            ...
            let mut node_as_set_accessor_declaration = self
                .factory()
                .create_set_accessor_declaration(decorators, modifiers, name, parameters, body);
    ...
    pub(super) fn parse_class_static_block_declaration(
        ...
        decorators: Option<Gc<NodeArray>>,
        modifiers: Option<Gc<NodeArray>>,
    ) -> Gc<Node /*ClassStaticBlockDeclaration*/> {
        self.parse_expected_token(SyntaxKind::StaticKeyword, None, None);
        let body: Gc<Node> = self.parse_class_static_block_body();
        self.with_jsdoc(
            self.finish_node(
                self.factory()
                    .create_class_static_block_declaration(decorators, modifiers, body),
                pos,
                None,
            )
            .wrap(),
    ...
    pub(super) fn parse_class_static_block_body(&self) -> Gc<Node /*Block*/> {
    ...
    pub(super) fn parse_decorator_expression(&self) -> Gc<Node /*LeftHandSideExpression*/> {
        if self.in_await_context() && self.token() == SyntaxKind::AwaitKeyword {
            let pos = self.get_node_pos();
            let await_expression: Gc<Node> = self
    ...
    pub(super) fn try_parse_decorator(&self) -> Option<Decorator> {
        ...
        Some(self.finish_node(self.factory().create_decorator(expression), pos, None))
    }

    pub(super) fn parse_decorators(&self) -> Option<Gc<NodeArray> /*<Decorator>*/> {
        let pos = self.get_node_pos();
        let mut list: Option<Vec<Gc<Node>>> = None;
        loop {
    ...
    pub(super) fn parse_modifiers_for_arrow_function(
        ...
        if self.token() == SyntaxKind::AsyncKeyword {
            ...
            let modifier = self
                .finish_node(
                    self.factory().create_token(SyntaxKind::AsyncKeyword),
                    pos,
                    None,
                )
                .wrap();
    ...
    pub(super) fn parse_class_element(&self) -> Gc<Node /*ClassElement*/> {
        let pos = self.get_node_pos();
        if self.token() == SyntaxKind::SemicolonToken {
            self.next_token();
            return self
                .finish_node(self.factory().create_semicolon_class_element(), pos, None)
                .wrap();
        }
            ...
            )
        {
            let is_ambient = some(
                ...
                Some(|modifier: &Gc<Node>| self.is_declare_modifier(modifier)),
            );
            if is_ambient {
    ...
    pub(super) fn parse_class_expression(&self) -> Gc<Node /*ClassExpression*/> {
